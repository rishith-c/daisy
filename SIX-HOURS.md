...
## 5. The physics gate made real too

With about an hour left I noticed the third pillar was still narrated: the taste
gate and Precedent were programs, but the physics numbers were hand-written. So
`hardware/margins.py` — closed-form bending, bolt shear, thermal, mass, and a
fastener selection driven by the scraped rows. 27 tests.

It found two things immediately, and the first is the uncomfortable one:

**The narrated physics was not physically real.** The run claimed 212 MPa at
FoS 0.82. The load case as stated actually gives **69.0 MPa at FoS 0.72** in
PETG — the story (fails, gets repaired, passes) was right, the numbers were
invented. Every figure in the run is now generated by the engine.

**The repair could fail its own gate.** `solve_thickness` rounded to nearest, so
a solved thickness could land fractionally under target — the repair failing the
