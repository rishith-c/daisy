...
check("reports a governed Daisy Chain topology", 'case "chain.status"' in SOURCE and
      '["python3", "labctl.py", "chain", "--project", projectPath, "--json"]' in SOURCE)
check("runs a real governed Daisy Chain goal", 'case "chain.run"' in SOURCE and
      all(part in SOURCE for part in ('["python3", "labctl.py", "run", "--brief", goal, "--lane", "crew"',
...
check("offers a native directory-only project picker", all(part in SOURCE for part in (
    'case "project.choose"', "NSOpenPanel()", "canChooseDirectories = true",
    "canChooseFiles = false", "allowedContentTypes = [.folder]", "window.__daisyProject")))
check("persists only a user-selected project directory", all(part in SOURCE for part in (
    'case "project.status"', '"daisy.project.path"', "FileManager.default.fileExists")))
check("scopes single and chain runs to the selected project", SOURCE.count('"--project", projectPath') >= 2)
...
check("turns an empty native failure into a visible callback", 'let payload = json.isEmpty' in SOURCE and 'Daisy could not complete the local command.' in SOURCE)
...
print(f"\n{28-len(failed)} passed, {len(failed)} failed")
